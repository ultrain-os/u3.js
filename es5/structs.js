...
module.exports = function () {
  ...
  var structLookup = function structLookup(lookupName, account) {
    var cachedCode = new Set(['ultrainio', 'utrio.token', 'utrio.null']);
  ...
  var customTypes = Object.assign({}, ultrainTypes, config.customTypes);
  ...
  var fullSchema = Object.assign({}, schema.schema, extendedSchema);
...
var NameEx = function NameEx(validation) {
  return {
    fromByteBuffer: function fromByteBuffer(b) {
      var valH = b.readUint64();
      var valL = b.readUint64();
      var n = decodeNameEx(valH, valL, false); // b is already in littleEndian
      // if(validation.debug) {
      //   console.error(`${n}`, '(Name.fromByteBuffer)')
      // }
      return n;
    ...
    appendByteBuffer: function appendByteBuffer(b, value) {
      ...
      //   console.error(`${value}`, (Name.appendByteBuffer))
      // }
      var result = encodeNameEx(value, false);
...
function precisionCache(assetCache, value) {
  ...
  var contract = symbolInfo.contract || 'utrio.token';
...
function toAssetString(value, assetCache) {
  ...
  var _precisionCache = precisionCache(assetCache, value),
      ...
      amount = _precisionCache.amount,
      contract = _precisionCache.contract;
...
var abiOverride = function abiOverride(structLookup) {
  return {
    ...
    'setabi.abi.appendByteBuffer': function setabiAbiAppendByteBuffer(_ref) {
      ...
      var ser = structLookup('abi_def', 'ultrainio');
