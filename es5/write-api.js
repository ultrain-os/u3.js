...
var processArgs = require("./utils/process-args");
...
function WriteApi(Network, network, config, Transaction) {
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 34:
              ...
              cachedCode = new Set(["ultrainio", "utrio.token", "ultrainio.null"]);
  ...
  function genMethod(type, definition, transactionArg) {
    var account = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "utrio.token";
  ...
  function transaction(arg, options, callback) {
    var defaultExpiration = config.expireInSeconds ? config.expireInSeconds : 60 * 60;
            ...
            case 11:
              ...
              Promise.all(sigs).then(function (sigs) {
                ...
                if (!options.broadcast || !network) {
                  ...
                  network.pushTx(packedTr, function (error) {
                    if (!error) {
                      callback(null, {
                        transaction_id: transactionId,
                        broadcast: true,
